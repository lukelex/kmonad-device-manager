...
var commandHelp = []cliCommandHelp{
	...
	},
	{
		...
		Description: "Ask the running manager for public API, manager, KMonad, platform, and backend version/compatibility information; per-instance server ID; health; state revision; resumable event cursor; configured public limits; capability availability; and feature limitations. This read does not discover devices, change configuration state, or affect reconciliation.",
		Options:     []cliOptionHelp{jsonOptionHelp},
		JSONOutput:  "Returns api_versions, manager_version, server_id, platform, platform_version, backend, backend_version, kmonad, state_revision, event_cursor, limits, capabilities, limitations, and health. Errors are JSON objects on standard error.",
		Examples: []string{
	...
	},
	{
        ...
        Description: "Ask the running manager for one coherent state view. The snapshot includes connected and known-disconnected devices, managed and read-only external configurations, retained operations, public diagnostics, manager health, and a monotonic state revision. It does not require or trigger an API client for reconciliation.",
		Options:     []cliOptionHelp{jsonOptionHelp},
	...
	{
		Name:        "inputscan",
		Invocation:  "kmonad-device-manager inputscan { get DEVICE_ID | probe DEVICE_ID TOKEN [--timeout SECONDS] | status OPERATION_ID | cancel OPERATION_ID } [--json]",
		Summary:     "Scan a device's key capabilities or probe one named key.",
		Description: "Read the versioned token set a connected keyboard's input node can emit (device.inputscan.get) or probe one named kmonad-v1 token by waiting for its keypress (probe). Scans are facts only: they never infer a layout, product, or geometry and never change a mapping. A probe pauses only the KMonad configuration using that device while it waits, then restores it after success, timeout, cancellation, or hotplug. Only one probe session may run at a time; unrelated keyboards continue running.",
		Arguments: []cliArgumentHelp{
			{Name: "DEVICE_ID", Description: "Opaque ID returned by devices; required by get and probe."},
			{Name: "TOKEN", Description: "kmonad-v1 token to probe, for example 102nd; required by probe."},
			{Name: "OPERATION_ID", Description: "Opaque ID returned by probe; required by status and cancel."},
		},
		Options: []cliOptionHelp{
			jsonOptionHelp,
			{Syntax: "--timeout SECONDS", Description: "Probe only. Wait from 1 through 30 seconds; the default is 15 seconds."},
		},
		JSONOutput: "get returns an input scan object with opaque device ID, token namespace, sorted key token set, unmapped count, evidence generation, deterministic digest, and observed timestamp. probe, status, and cancel return an operation object like identify. Errors are JSON objects on standard error.",
		Examples: []string{
	...
	},
	{
		...
		Arguments: []cliArgumentHelp{
			...
			{Name: "CONTENT_REVISION", Description: "Positive content_revision token from list required by read; rejects changed external source."},
			{Name: "manager_rendered_kbd", Description: "Only supported export format; the result is bound to this manager and its selected keyboard."},
